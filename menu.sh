...
case $RETVAL in
    ...
    "8") clear
		...
		echo "Delete motion video and image files."
		#while true; 
		#do
		echo "${GREEN}Running chmod instruction"
		sudo chmod a+rrwx /var/lib/motion
		#echo "${GREEN}Running delete for /var/lib/motion/"
		# sudo rm /var/lib/motion/* # failed argument too long
		echo "${GREEN}Running delete for var/lib/motion"
		sudo rsync -a --delete empty/ var/lib/motion/
		
		#echo "${GREEN}Running delete for *.mkv"
		#sudo find . -name "*.mkv" -print0 | xargs -0 rm
		
		#echo "${GREEN}Running delete for *.avi"
		#sudo find . -name "*.avi" -print0 | xargs -0 rm
		#done
